...
def test_invalid_channel():
    ...
    assert addowner_ch.status_code == 400
...
def test_already_owner():
    ...
    addowner1_ch = requests.post(f"{BASE_URL}/channel/addowner/v1", json = {
        'token':payload0['token'],
        'channel_id': payload2['channel_id'],
        'u_id': payload1['auth_user_id'],
    })

    addowner2_ch = requests.post(f"{BASE_URL}/channel/addowner/v1", json = {
        'token':payload0['token'],
        'channel_id': payload2['channel_id'],
        'u_id': payload1['auth_user_id'],
    })

    assert addowner1_ch.status_code == 200
    assert addowner2_ch.status_code == 400
