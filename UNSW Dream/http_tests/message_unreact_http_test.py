...
def test_invalid_message_id_within_channel():
    ...
    unreact_stat = requests.post(f"{BASE_URL}/message/unreact/v1", json = {
        'token': payload0['token'],
        'message_id': 2,
        'react_id' : 1,
    })
    assert unreact_stat.status_code == 400
...
def test_already_unreacted_ch():
    ...
    reunreact_stat = requests.post(f"{BASE_URL}/message/unreact/v1", json = {
        'token': payload0['token'],
        'message_id': payload2['message_id'],
        'react_id' : 1,
    })
    assert react_stat.status_code == 200
    assert unreact_stat.status_code == 200
    assert reunreact_stat.status_code == 400
...
def test_user_not_in_ch():
    ...
    assert unreact_stat.status_code == 403

def test_unreact_success_ch():
    requests.delete(f"{BASE_URL}/clear/v1")
    user = requests.post(f"{BASE_URL}/auth/register/v2", json={
        'email': 'z5270707@gmail.com',
        'password': 'z5270707',
