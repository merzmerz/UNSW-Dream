...
def test_invalid_channel():
    ...
    with pytest.raises(src.error.InputError):
        channel_addowner_v1(user_id1['token'], 5, user_id2['auth_user_id'])

'''
test2 user with user id u_id already an owner of the channel
'''
...
def test_addowner_uidNone():
    ...
    with pytest.raises(src.error.InputError):
        channel_addowner_v1(user_id1['token'], channel_create['channel_id'], None)

'''
AccessError case
test1 Authorized user is not an owner of the channel or DREAM
'''
def test_not_owner():
    clear()
    user_id1 = auth_register_v2('firstemail@gmail.com', '12345Hello', 'John', 'Snow')
    ...
    with pytest.raises(src.error.AccessError):
        channel_addowner_v1(user_id3['token'], channel_create['channel_id'], user_id2['auth_user_id'])
...
def test_token_invalid():
    ...
    with pytest.raises(src.error.AccessError):
        channel_addowner_v1(user_id1['token'] + "abc", ch_id, user_id2['auth_user_id'])
...
def test_addowner_success():
    ...
    assert {'u_id': 2, 'name_first': 'Mikey', 'name_last': 'Mouse', 'email': 'secondemail@gmail.com', 'handle_str': 'mikeymouse', 'profile_img_url': f"http://localhost:{PORT}/static/initial.jpg"} == data['channels'][0]['owner_members'][1]
